...
#if defined(NDEBUG)
# error "Diamint cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Diamint Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s DIAM additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
	...
	CAmount nSubsidy;
	if (nHeight ==1)
		nSubsidy = 166326940 * COIN;
	else
    CAmount nSubsidy = 6931 * COIN;
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    if (nCheckDepth <= 0)
        // Diamint: suffices until year 10214. Didn't x4 value due to integer wrap around and upstream compatibility.
